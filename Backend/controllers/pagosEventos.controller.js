...
const Peventos_models_1 = require("../models/pagosEventos.models");
const manage_error_1 = require("../error/manage.error");

const getPEventos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const PeventosList = yield Peventos_models_1.PagosEvents.findAll();
        res.json({
            PeventosList
        });
    }
...
const getPEventoById = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const Pevento = yield Peventos_models_1.PagosEvents.findOne({ where: { id: idPEvento } });
        if (!Pevento) {
...
const newPEventos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const existPEventos = yield Peventos_models_1.PagosEvents.findOne({ where: { id: id } });
    if (existPEventos) {
    ...
    try {
        yield Peventos_models_1.PEvents.create({
            id: id,
            id_evento: id_evento,
        ...
        res.json({
            msg: `El Detalle del pago ${id} se creó satisfactoriamente`
        });
    }
    catch (error) {
        return res.status(500).json({
...
const updatePEventos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const idPEvento = req.params.id;
    const { id, id_evento, fechaEvento, id_detalle_pago } = req.body;
    const existPEventos = yield Peventos_models_1.PagosEvents.findOne({ where: { id: idPEvento } });
    if (!existPEventos) {
        return res.status(404).json({
            msg: manage_error_1.ErrorMessages.CLUB_NOT_FOUND
        });
    ...
    try {
        ...
        res.json({
            msg: `El pago del evento ${existPEventos.id} ha sido editado satisfactoriamente`
        });
    }
    catch (error) {
        return res.status(500).json({
...
const deletePEventos = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const existPEventos = yield Peventos_models_1.PagosEvents.findOne({ where: { id: idPEventos } });
    if (!existPEventos) {
        return res.status(404).json({
            msg: manage_error_1.ErrorMessages.CLUB_NOT_FOUND
        });
    ...
    try {
        yield Peventos_models_1.DEvents.destroy({ where: { id: idPEventos } });
        res.json({
            msg: `El pago del evento ${existPEventos.id} ha sido removido satisfactoriamente`
        });
    }
    catch (error) {
        return res.status(500).json({
